-- Ensure table and extension exist (no-op if already created)
...
-- SAFER FUNCTION: security invoker (default), RLS enforced
drop function if exists public.match_manual_chunks(vector(1536), int);
...
grant execute on function public.match_manual_chunks(vector(1536), int) to anon, authenticated;